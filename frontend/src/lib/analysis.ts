import type { AdPerformance, AnalysisRecord, UploadStats, UserSession } from "@/types";

export function getGrowScore(stats: UploadStats) {
...
export function getQualityLabel(score: number) {
  ...
  if (score >= 660) return "Promising";
  if (score >= 580) return "Needs tuning";
  return "Needs work";
}

export function buildRecommendation(stats: UploadStats, growScore: number) {
...
export function buildNextActions(stats: UploadStats, growScore: number) {
  const resultAds = getTopResultAds(stats);
  const adNames = resultAds.map((ad) => ad.ad).join(", ");
  const actions = [
...
export function getAdDetails(stats: UploadStats): AdPerformance[] {
  if (stats.adDetails?.length) return stats.adDetails;
...
export function getRankedAds(stats: UploadStats) {
...
export function getTopResultAds(stats: UploadStats) {
  return getRankedAds(stats);
}

export function getBudgetAllocation(stats: UploadStats, totalBudget: number) {
...
export function getAdDecision(ad: AdPerformance, rank: number, averageCtr: number) {
  if (rank === 0) {
    return {
      label: "Spend first",
      tone: "lime" as const,
...
export function buildAnalysisRecord(
  ...
  source: AnalysisRecord["source"] = "upload"
): AnalysisRecord {
...
export function buildStatsFromManualRows(rows: AdPerformance[], filename = "manual-ad-performance"): UploadStats {
