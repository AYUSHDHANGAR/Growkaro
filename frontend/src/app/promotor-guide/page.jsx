...
export default function PromotorGuidePage() {
    return (<AppShell eyebrow="Helping new promoters" title="Target marketing guide" description="Choose where to promote based on objective, CTR benchmark, cost signal, and the type of customer you need.">
      <AuthGate title="Login required for the marketing guide" description="Sign in before viewing channel benchmarks, cost guidance, and saved promotion recommendations.">
        <section className="mx-auto grid w-full max-w-7xl gap-5 px-5 pb-16">
          <div className="grid gap-4 lg:grid-cols-5">
            {promoterGuides.map((guide) => (<article key={guide.title} className="glass-panel rounded-lg p-5">
        ...
        <section className="glass-panel rounded-lg p-5">
          ...
          <div className="mt-5 overflow-x-auto">
            <table className="w-full min-w-[900px] border-separate border-spacing-y-2 text-left text-sm">
              ...
              <tbody>
                {platformBenchmarks.map((platform) => (<tr key={platform.channel} className="bg-white/[0.04]">
        ...
        ].map(([title, body]) => (<article key={title} className="glass-panel rounded-lg p-5">
