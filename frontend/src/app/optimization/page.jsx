...
export default function OptimizationPage() {
    return (<AppShell eyebrow="Optimization workspace" title="Ad optimization with saved history" description="Upload ad performance, review UCB recommendations, compare channels, and keep every analysis saved for the current user.">
      <AuthGate title="Login required for optimization" description="Sign in before uploading ad data, viewing channel guidance, or opening saved optimization history.">
        <section className="mx-auto grid w-full max-w-7xl gap-5 px-5 pb-4 xl:grid-cols-[1fr_360px]">
          <div className="glass-panel rounded-lg p-5">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {bestValueChannels.slice(0, 4).map((channel) => (<article key={channel.channel} className="rounded-lg border border-white/10 bg-white/[0.04] p-4">
