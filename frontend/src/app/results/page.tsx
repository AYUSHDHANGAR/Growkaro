...
import type { AdPerformance, AnalysisRecord, UserSession } from "@/types";

const emptyRows: AdPerformance[] = [
...
export default function ResultsPage() {
  ...
  const [manualRows, setManualRows] = useState<AdPerformance[]>(emptyRows);
  ...
  function updateManualRow(index: number, key: keyof AdPerformance, value: string) {
    setManualRows((rows) =>
  ...
  return (
    <AppShell
      eyebrow="Client result page"
      title="Detailed ad performance result"
      description="Each user can upload or enter their own ad numbers, receive a clear GrowScore result, and save a separate report history."
    >
      {!checkedAuth || !session ? (
      ...
      <section className="mx-auto grid w-full max-w-7xl gap-5 px-5 pb-16 xl:grid-cols-[1fr_360px]">
        <div className="grid gap-5">
            ...
            <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-lg border border-limeSignal/30 bg-limeSignal/10 px-4 py-3 font-black text-limeSignal"
                >
          ...
          <section className="glass-panel rounded-lg p-5">
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                type="button"
                onClick={() => setManualRows((rows) => [...rows, { ad: `Ad ${rows.length + 1}`, impressions: 0, clicks: 0, ctr: 0 }])}
                className="grid h-10 w-10 place-items-center rounded-lg border border-white/10 bg-white/[0.05] text-white"
                aria-label="Add ad row"
                title="Add ad row"
              >
              ...
              <table className="w-full min-w-[620px] border-separate border-spacing-y-2 text-left text-sm">
                ...
                <tbody>
                  {manualRows.map((row, index) => {
                    ...
                    return (
                      <tr key={`${row.ad}-${index}`} className="bg-white/[0.04]">
                        ...
                        <td className="px-3 py-2">
                          <input
                            ...
                            min={0}
                            type="number"
                            onChange={(event) => updateManualRow(index, "impressions", event.target.value)}
                            className="w-full rounded-lg border border-white/10 bg-midnight/60 px-3 py-2 outline-none"
                          />
                        </td>
                        <td className="px-3 py-2">
                          <input
                            ...
                            min={0}
                            type="number"
                            onChange={(event) => updateManualRow(index, "clicks", event.target.value)}
                            className="w-full rounded-lg border border-white/10 bg-midnight/60 px-3 py-2 outline-none"
                          />
                        </td>
