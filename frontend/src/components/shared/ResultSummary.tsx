...
type ResultSummaryProps = {
  ...
  onExportPdf?: (budget: number) => void;
};

export function ResultSummary({ record, onExportPdf }: ResultSummaryProps) {
  ...
  return (
    <section className="grid gap-5">
      <div className="glass-panel rounded-lg p-5">
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
          ...
          <div className="flex flex-col gap-3 sm:flex-row lg:flex-col">
            ...
            {onExportPdf && (
              <button
                type="button"
                onClick={() => onExportPdf(totalBudget)}
                className="inline-flex items-center justify-center gap-2 rounded-lg bg-limeSignal px-4 py-3 font-black text-midnight"
              >
        ...
        {topAds.length > 0 && (
          <div className="mt-5 rounded-lg border border-limeSignal/25 bg-limeSignal/10 p-5">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          {cards.map((card) => (
            <article key={card.label} className="rounded-lg border border-white/10 bg-white/[0.04] p-4">
      ...
      <section className="glass-panel rounded-lg p-5">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {topAds.map((ad, index) => {
            ...
            return (
              <article key={ad.ad} className="rounded-lg border border-white/10 bg-white/[0.04] p-4">
          ...
          <div>
            ...
            <label className="mt-4 grid gap-2 text-sm font-bold text-white/70">
              ...
              <input
                ...
                className="rounded-lg border border-white/10 bg-midnight/70 px-3 py-3 text-white outline-none"
              />
            </label>
          </div>
          <div className="grid gap-3">
            {budgetPlan.map((ad, index) => (
              <article key={ad.ad} className="rounded-lg border border-white/10 bg-white/[0.04] p-4">
            ...
            <table className="w-full min-w-[620px] border-separate border-spacing-y-2 text-left text-sm">
              ...
              <tbody>
                {topAds.map((ad, index) => {
                  const decision = getAdDecision(ad, index, averageCtr);
                  return (
                    <tr key={ad.ad} className="bg-white/[0.04]">
        ...
        <aside className="glass-panel rounded-lg p-5">
          ...
          <div className="mt-4 grid gap-3">
            {record.nextActions.map((action) => (
              <p key={action} className="rounded-lg border border-white/10 bg-white/[0.04] p-3 text-sm leading-6 text-white/64">
                {action}
              </p>
            ))}
          </div>
