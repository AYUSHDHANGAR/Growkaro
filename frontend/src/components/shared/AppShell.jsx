...
export function AppShell({ children, eyebrow, title, description }) {
    const router = useRouter();
    const [session, setSessionState] = useState(null);
    const visibleNavigation = session ? navigation.filter((item) => item.href !== "/admin" || session.role === "admin") : [];
    useEffect(() => {
    ...
    return (<main className="relative min-h-screen overflow-hidden bg-midnight">
      <GsapMotion />
      <div className="noise"/>
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-midnight/86 backdrop-blur-xl">
        <nav className="mx-auto flex w-full max-w-7xl flex-col gap-4 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center justify-between gap-4">
            ...
            <Link href="/optimization" className="grid h-10 w-10 place-items-center rounded-lg border border-cyanEdge/30 bg-cyanEdge/10 text-cyanEdge lg:hidden" aria-label="Open optimization">
              <LayoutDashboard className="h-5 w-5"/>
            </Link>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {visibleNavigation.map((item) => (<Link key={item.href} href={item.href} className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-sm font-bold text-white/70 transition hover:border-limeSignal/50 hover:bg-limeSignal/10 hover:text-white">
                <item.icon className="h-4 w-4"/>
                {item.label}
              </Link>))}
            {session ? (<button type="button" onClick={handleLogout} className="inline-flex items-center gap-2 rounded-lg border border-roseEdge/30 bg-roseEdge/10 px-3 py-2 text-sm font-bold text-rose-100">
                ...
                <Link href="/signup" className="inline-flex items-center gap-2 rounded-lg border border-white/15 bg-white/[0.05] px-3 py-2 text-sm font-black text-white">
                  Sign up
                </Link>
              </>)}
          </div>
      ...
      {(title || description || eyebrow) && (<section data-gsap="fade-up" className="mx-auto w-full max-w-7xl px-5 pb-6 pt-10">
