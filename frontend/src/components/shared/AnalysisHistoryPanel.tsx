...
export function AnalysisHistoryPanel({ scope = "user", limit = 8 }: AnalysisHistoryPanelProps) {
  ...
  return (
    <section className="glass-panel rounded-lg p-5">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          type="button"
          onClick={() => exportHistoryCsv(records)}
          disabled={!records.length}
          className="grid h-10 w-10 place-items-center rounded-lg border border-white/10 bg-white/[0.05] text-white/70 disabled:opacity-40"
      ...
      <div className="mt-4 grid gap-3">
        {visibleRecords.length ? (
          visibleRecords.map((record) => {
            ...
            return (
            <article key={record.id} className="rounded-lg border border-white/10 bg-white/[0.04] p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  ...
                  className="grid h-8 w-8 place-items-center rounded-lg border border-roseEdge/25 bg-roseEdge/10 text-rose-100"
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {topAds.map((ad) => (
                  <span key={ad.ad} className="rounded-lg border border-limeSignal/20 bg-limeSignal/10 px-2 py-1 text-xs font-black text-limeSignal">
                    #{ad.rank} {ad.ad}
                  </span>
                ))}
              </div>
