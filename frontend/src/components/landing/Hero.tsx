...
export function Hero() {
  return (
    <section className="relative overflow-hidden pb-20 pt-8 lg:pb-24">
      <GsapMotion />
      <nav className="relative z-10 mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-5">
        ...
        <div className="hidden items-center gap-7 text-sm font-semibold text-white/65 md:flex">
          ...
          <Link href="/promotor-guide">Promoter Guide</Link>
          <Link href="/ad-creation">Ad Creation</Link>
          <Link href="/admin">Admin</Link>
        </div>
        <Link
          href="/login"
          className="rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm font-bold text-white shadow-glow backdrop-blur transition hover:border-cyanEdge/60"
        >
          Login
        </Link>
      </nav>
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-7xl items-center gap-10 px-5 pt-12 lg:grid-cols-[1fr_0.95fr]">
        <motion.div data-gsap="fade-up" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Link
              href="/results"
              className="inline-flex items-center justify-center gap-2 rounded-full bg-cyanEdge px-6 py-3 font-black text-midnight shadow-glow transition hover:bg-white"
            >
              Start growth check
            ...
            <Link
              href="/optimization"
              className="inline-flex items-center justify-center gap-2 rounded-full border border-white/15 bg-white/10 px-6 py-3 font-black text-white backdrop-blur transition hover:border-violetEdge/60"
            >
            ...
            <div className="grid gap-3 sm:grid-cols-[180px_1fr]">
              <div className="rounded-3xl border border-white/10 bg-white/[0.05] p-5">
                <div
                  ...
                  style={{ background: "conic-gradient(#74f7b6 0deg 292deg, rgba(255,255,255,.12) 292deg 360deg)" }}
                >
              ...
              <div className="grid content-start gap-3 rounded-3xl border border-white/10 bg-white/[0.04] p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {targetChannels.map((channel) => (
                    <span key={channel} className="rounded-full border border-white/10 bg-white/[0.06] px-3 py-1 text-xs font-bold text-white/70">
                      {channel}
                    </span>
                  ))}
                </div>
