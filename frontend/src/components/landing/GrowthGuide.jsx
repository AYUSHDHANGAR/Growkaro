...
export function GrowthGuide() {
    return (<section id="guide" className="mx-auto w-full max-w-7xl px-5 py-16">
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {targetGroups.map((group, index) => (<motion.article key={group.title} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ delay: index * 0.06 }} className="glass-panel rounded-3xl p-6">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_360px]">
        <div className="glass-panel rounded-3xl p-6">
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-4">
            {["Quality proof", "Local reach", "Best ad", "Next test"].map((step, index) => (<div key={step} className="rounded-2xl border border-white/10 bg-white/[0.05] p-4">
        ...
        <aside className="glass-panel rounded-3xl p-6">
          ...
          <div className="mt-5 grid gap-3">
            {recommendations.map((item) => (<div key={item} className="flex gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-sm leading-6 text-white/64">
