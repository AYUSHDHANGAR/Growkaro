...
export function DashboardShell() {
    ...
    function handleStats(nextStats) {
    ...
    if (!isAuthenticated) {
        return (<section id="dashboard" className="mx-auto w-full max-w-7xl px-5 py-16">
    ...
    return (<section id="dashboard" className="mx-auto w-full max-w-7xl px-5 py-16">
      <div className="mb-8 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
        ...
        {stats ? (<div className="rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm font-bold text-white/68">
            Active dataset: <span className="text-white">{stats.filename}</span>
          </div>) : (<div className="rounded-full border border-amberSignal/25 bg-amberSignal/10 px-4 py-2 text-sm font-bold text-amberSignal">No dataset selected</div>)}
      </div>

      <DatasetUpload onStats={handleStats}/>

      {!stats ? (<div className="mt-5 rounded-lg border border-dashed border-white/15 bg-white/[0.03] p-8 text-center">
          ...
          <aside className="glass-panel mt-5 rounded-3xl p-6">
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-3">
              {buildNextActions(stats, growScore ?? 300).map((action, index) => (<article key={action} className={`rounded-2xl border p-4 ${toneClass[index === 0 ? "cyan" : index === 1 ? "lime" : "violet"]}`}>
