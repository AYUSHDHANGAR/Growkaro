...
export function PerformanceMeter({ stats }: PerformanceMeterProps) {
  ...
  return (
    <section id="performance-score" className="glass-panel rounded-3xl p-6">
      ...
      <div className="grid gap-5 lg:grid-cols-[280px_1fr_320px]">
        <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
          <div
            className="mx-auto grid aspect-square max-w-[220px] place-items-center rounded-full p-4"
            style={{ background: `conic-gradient(#74f7b6 0deg ${scoreDegrees}deg, rgba(255,255,255,.1) ${scoreDegrees}deg 360deg)` }}
          >
          ...
          <div className="mt-5 grid grid-cols-2 gap-2">
            {scoreBands.map((band) => (
              <div key={band.label} className="rounded-2xl border border-white/10 bg-white/[0.04] p-3">
        ...
        <div className="grid gap-3">
          {meters.map((meter) => (
            <div key={meter.label} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
